...
N <- 10
T_int <- c(100,250,500)
file_name <- "./results/temp_spline_infofriction.Rda"
...
file_name <- "./results/final_spline_infofriction.Rda"
...
inst.fore2 <- paste0("lag(",
                     c("X", "lag(X-Y)", "lag(X-Y)^2", "lag(lag(X-Y))", "lag(X)", "lag(lag(X-Y)^2)"),
                     ")")
...
simulate(save_to = file_name, T_int=T_int, N=N, dgp = dgp,
         tests = list(
           ...
           list(description = "spline_lag", iden.fct = spline.flex.median ,model = probit_linear, instruments= inst.fore2, state = "lag(lag(Y))"),
           list(description = "quantileslin_Y_lag", iden.fct = quantiles ,model = probit_linear, instruments= inst.fore2, state = "lag(lag(Y))"),
           list(description = "expectileslin_Y_lag", iden.fct = expectiles ,model = probit_linear, instruments= inst.fore2, state = "lag(lag(Y))")
         ),forecasters = list(
...
plot.on.T(file_name,nice.names = TRUE)
